...
(function ($) {
    ...
    $.JQPlugin.createPlugin({name: k, defaultOptions: {showOn: 'focus', buttonImage: '', buttonImageOnly: false, showAnim: 'show', showOptions: null, duration: 'normal', appendText: '', useThemeRoller: false, keypadClass: '', prompt: '', layout: [], separator: '', target: null, keypadOnly: true, randomiseAlphabetic: false, randomiseNumeric: false, randomiseOther: false, randomiseAll: false, beforeShow: null, onKeypress: null, onClose: null}, regionalOptions: {'': {buttonText: '...', buttonStatus: 'Open the keypad', closeText: 'Close', closeStatus: 'Close the keypad', clearText: 'Clear', clearStatus: 'Erase all the text', backText: 'Back', backStatus: 'Erase the previous character', spacebarText: '&#160;', spacebarStatus: 'Space', enterText: 'Enter', enterStatus: 'Carriage return', tabText: '→', tabStatus: 'Horizontal tab', shiftText: 'Shift', shiftStatus: 'Toggle upper/lower case characters', alphabeticLayout: [], fullLayout: [], isAlphabetic: null, isNumeric: null, toUpper: null, isRTL: false}}, _getters: ['isDisabled'], _curInst: null, _disabledFields: [], _keypadShowing: false, _keyCode: 0, _specialKeys: [], _mainDivClass: k + '-popup', _inlineClass: k + '-inline', _appendClass: k + '-append', _triggerClass: k + '-trigger', _disableClass: k + '-disabled', _inlineEntryClass: k + '-keyentry', _rtlClass: k + '-rtl', _rowClass: k + '-row', _promptClass: k + '-prompt', _specialClass: k + '-special', _namePrefixClass: k + '-', _keyClass: k + '-key', _keyDownClass: k + '-key-down', qwertyAlphabetic: ['qwertyuiop', 'asdfghjkl', 'zxcvbnm'], qwertyLayout: ['!@#$%^&*()_=' + this.HALF_SPACE + this.SPACE + this.CLOSE, this.HALF_SPACE + '`~[]{}<>\\|/' + this.SPACE + '789', 'qwertyuiop\'"' + this.HALF_SPACE + '456', this.HALF_SPACE + 'asdfghjkl;:' + this.SPACE + '123', this.SPACE + 'zxcvbnm,.?' + this.SPACE + this.HALF_SPACE + '-0+', '' + this.TAB + this.ENTER + this.SPACE_BAR + this.SHIFT + this.HALF_SPACE + this.BACK + this.CLEAR], addKeyDef: function (a, b, c, d) {
        ...
        this._specialKeys.push({code: this[a], id: a, name: b, action: c, noHighlight: d});
        return this
    }, _init: function () {
        this.mainDiv = $('<div class="' + this._mainDivClass + '" style="display: none;"></div>');
        this._super()
    }, _instSettings: function (a, b) {
        var c = !a[0].nodeName.toLowerCase().match(/input|textarea/);
        return{_inline: c, ucase: false, _mainDiv: (c ? $('<div class="' + this._inlineClass + '"></div>') : m.mainDiv)}
        ...
        b._input = $(!b._inline ? a : b.options.target || '<input type="text" class="' + this._inlineEntryClass + '" disabled/>');
        if (b._inline) {
            a.find('input').remove();
            if (!b.options.target) {
                a.append(b._input)
        ...
        $.extend(e.options, f);
        d.off('.' + e.name).siblings('.' + this._appendClass).remove().end().siblings('.' + this._triggerClass).remove();
        var g = e.options.appendText;
        if (g) {
            d[e.options.isRTL ? 'before' : 'after']('<span class="' + this._appendClass + '">' + g + '</span>')
        }
        if (!e._inline) {
            if (e.options.showOn == 'focus' || e.options.showOn == 'both') {
                d.on('focus.' + e.name, this.show).on('keydown.' + e.name, this._doKeyDown)
            }
            if (e.options.showOn == 'button' || e.options.showOn == 'both') {
                var h = e.options.buttonStatus;
                var i = e.options.buttonImage;
                var j = $(e.options.buttonImageOnly ? $('<img src="' + i + '" alt="' + h + '" title="' + h + '"/>') : $('<button type="button" title="' + h + '"></button>').html(i == '' ? e.options.buttonText : $('<img src="' + i + '" alt="' + h + '" title="' + h + '"/>')));
        ...
        d[e.options.keypadOnly ? 'attr' : 'removeAttr']('readonly', true).on('setData.' + e.name, function (a, b, c) {
            e.options[b] = c
        ...
        a.siblings('.' + this._appendClass).remove().end().siblings('.' + this._triggerClass).remove().end().prev('.' + this._inlineEntryClass).remove();
        a.empty().off('.' + b.name)[b.saveReadonly ? 'attr' : 'removeAttr']('readonly', true);
        ...
        if (c.match(/input|textarea/)) {
            b.prop('disabled', false).siblings('button.' + this._triggerClass).prop('disabled', false).end().siblings('img.' + this._triggerClass).css({opacity: '1.0', cursor: ''})
        } else if (c.match(/div|span/)) {
            b.children('.' + this._disableClass).remove();
            this._getInst(b)._mainDiv.find('button').prop('disabled', false)
        }
        ...
        var c = b[0].nodeName.toLowerCase();
        if (c.match(/input|textarea/)) {
            b.prop('disabled', true).siblings('button.' + this._triggerClass).prop('disabled', true).end().siblings('img.' + this._triggerClass).css({opacity: '0.5', cursor: 'default'})
        } else if (c.match(/div|span/)) {
            ...
            b.prepend('<div class="' + this._disableClass + '" style="width: ' + d.outerWidth() + 'px; height: ' + d.outerHeight() + 'px; left: ' + (e.left - f.left) + 'px; top: ' + (e.top - f.top) + 'px;"></div>');
            this._getInst(b)._mainDiv.find('button').prop('disabled', true)
        }
        this._disabledFields = $.map(this._disabledFields, function (a) {
            return(a == b[0] ? null : a)
        });
        ...
        var d = {left: m._pos[0], top: m._pos[1]};
        m._pos = null;
        b._mainDiv.css({position: 'absolute', display: 'block', top: '-1000px', width: 'auto'});
        m._updateKeypad(b);
        d = m._checkOffset(b, d, c);
        b._mainDiv.css({position: (c ? 'fixed' : 'absolute'), display: 'none', left: d.left + 'px', top: d.top + 'px'});
        var e = b.options.duration;
        var f = b.options.showAnim;
        var g = function () {
            m._keypadShowing = true
        };
        if ($.effects && ($.effects[f] || ($.effects.effect && $.effects.effect[f]))) {
            var h = b._mainDiv.data();
            for (var i in h) {
                if (i.match(/^ec\.storage\./)) {
                    h[i] = b._mainDiv.css(i.replace(/ec\.storage\./, ''))
                }
            }
            b._mainDiv.data(h).show(f, b.options.showOptions || {}, e, g)
        } else {
        ...
        a._mainDiv.empty().append(this._generateHTML(a)).removeClass().addClass(a.options.keypadClass + (a.options.useThemeRoller ? ' ui-widget ui-widget-content' : '') + (a.options.isRTL ? ' ' + this._rtlClass : '') + ' ' + (a._inline ? this._inlineClass : this._mainDivClass));
        if ($.isFunction(a.options.beforeShow)) {
            a.options.beforeShow.apply((a._input ? a._input[0] : null), [a._mainDiv, a])
        }
        ...
        var i = 0;
        a._mainDiv.find(':not(div)').each(function () {
            i = Math.max(i, this.offsetLeft + $(this).outerWidth(true))
        });
        a._mainDiv.css('width', i + 1);
        if (a.options.isRTL || (b.left + a._mainDiv.outerWidth() - g) > e) {
            b.left = Math.max((c ? 0 : g), d[0] + (a._input ? a._input.outerWidth() : 0) - (c ? g : 0) - a._mainDiv.outerWidth())
        } else {
            b.left = Math.max((c ? 0 : g), b.left - (c ? g : 0))
        }
        if ((b.top + a._mainDiv.outerHeight() - h) > f) {
            b.top = Math.max((c ? 0 : h), d[1] - (c ? h : 0) - a._mainDiv.outerHeight())
        } else {
        ...
        if (this._keypadShowing) {
            ...
            if ($.effects && ($.effects[d] || ($.effects.effect && $.effects.effect[d]))) {
                ...
                c._mainDiv[(d == 'slideDown' ? 'slideUp' : (d == 'fadeIn' ? 'fadeOut' : 'hide'))](d ? b : 0)
            }
        }
        if ($.isFunction(c.options.onClose)) {
            c.options.onClose.apply((c._input ? c._input[0] : null), [c._input.val(), c])
        }
        if (this._keypadShowing) {
        ...
        var b = $(a.target);
        if (b.closest('.' + m._mainDivClass).length === 0 && !b.hasClass(m._getMarker()) && b.closest('.' + m._triggerClass).length === 0 && m._keypadShowing) {
            m.hide()
        }
        ...
        d = (a._input.prop('readonly') || a._input.prop('disabled') ? d : (b.setSelectionRange ? [b.selectionStart, b.selectionEnd] : (b.createTextRange ? this._getIERange(b) : d)));
        this._setValue(a, (c.length == 0 ? '' : c.substr(0, d[0] - 1) + c.substr(d[1])), d[0] - 1);
        ...
        e = (a.attr('readonly') || a.attr('disabled') ? e : (c.setSelectionRange ? [c.selectionStart, c.selectionEnd] : (c.createTextRange ? this._getIERange(c) : e)));
        ...
        var c = (b ? a.createTextRange() : document.body.createTextRange());
        if (!b) {
        ...
        if ($.isFunction(a.options.onKeypress)) {
            a.options.onKeypress.apply((a._input ? a._input[0] : null), [b, a._input.val(), a])
        }
    }, _generateHTML: function (b) {
        var c = (!b.options.prompt ? '' : '<div class="' + this._promptClass + (b.options.useThemeRoller ? ' ui-widget-header ui-corner-all' : '') + '">' + b.options.prompt + '</div>');
        ...
        for (var i = 0; i < d.length; i++) {
            ...
            for (var j = 0; j < e.length; j++) {
                ...
                if (f) {
                    c += (f.action ? '<button type="button" class="' + this._specialClass + ' ' + this._namePrefixClass + f.name + (b.options.useThemeRoller ? ' ui-corner-all ui-state-default' + (f.noHighlight ? '' : ' ui-state-highlight') : '') + '" title="' + b.options[f.name + 'Status'] + '">' + (b.options[f.name + 'Text'] || '&#160;') + '</button>' : '<div class="' + this._namePrefixClass + f.name + '"></div>')
                } else {
                    c += '<button type="button" class="' + this._keyClass + (b.options.useThemeRoller ? ' ui-corner-all ui-state-default' : '') + '">' + (e[j] == ' ' ? '&#160;' : e[j]) + '</button>'
                }
            }
            c += '</div>'
        }
        c = $(c);
        var g = b;
        var h = this._keyDownClass + (b.options.useThemeRoller ? ' ui-state-active' : '');
        ...
        $.each(this._specialKeys, function (i, a) {
            c.find('.' + m._namePrefixClass + a.name).click(function () {
                a.action.apply(g._input, [g])
            })
        });
        return c
    }, _randomiseLayout: function (b) {
        if (!b.options.randomiseNumeric && !b.options.randomiseAlphabetic && !b.options.randomiseOther && !b.options.randomiseAll) {
        ...
        for (var i = 0; i < b.options.layout.length; i++) {
            var g = b.options.layout[i].split(b.options.separator);
            for (var j = 0; j < g.length; j++) {
                f[i] += (this._isControl(g[j]) ? g[j] : (b.options.randomiseAll ? e[o++] : (b.options.isNumeric(g[j]) ? c[n++] : (b.options.isAlphabetic(g[j]) ? d[a++] : e[o++])))) + b.options.separator
            }
        }
        return f
    ...
    m.numericLayout = ['123' + m.CLOSE, '456' + m.CLEAR, '789' + m.BACK, m.SPACE + '0'];
    m.qwertyLayout = ['!@#$%^&*()_=' + m.HALF_SPACE + m.SPACE + m.CLOSE, m.HALF_SPACE + '`~[]{}<>\\|/' + m.SPACE + '789', 'qwertyuiop\'"' + m.HALF_SPACE + '456', m.HALF_SPACE + 'asdfghjkl;:' + m.SPACE + '123', m.SPACE + 'zxcvbnm,.?' + m.SPACE + m.HALF_SPACE + '-0+', '' + m.TAB + m.ENTER + m.SPACE_BAR + m.SHIFT + m.HALF_SPACE + m.BACK + m.CLEAR], $.extend(m.regionalOptions[''], {alphabeticLayout: m.qwertyAlphabetic, fullLayout: m.qwertyLayout, isAlphabetic: m.isAlphabetic, isNumeric: m.isNumeric, toUpper: m.toUpper});
    m.setDefaults($.extend({layout: m.numericLayout}, m.regionalOptions['']));
    $(function () {
        $(document.body).append(m.mainDiv).on('mousedown.' + k, m._checkExternalClick)
    })
})(jQuery);