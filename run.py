...
def show_index():
    db = get_db()
    db = db.execute(
        '''SELECT user.name AS name, image_path,
        balance FROM user, account_valuable_balance AS avb WHERE active=1
        AND browsable=1 AND user.account_id = avb.account_id AND valuable_id = ?
...
def show_userpage(username):
    ...
    if not user:
        abort(404)
    cur = db.execute(
        'SELECT name, image_path FROM user WHERE active=1 AND direct_payment=0 AND browsable=1 AND name!=?',
        [username])
    ...
    cur = cur.execute(
        '''SELECT valuable.name AS name, price, unit_name, symbol,
        image_path FROM valuable, unit WHERE unit.name = valuable.unit_name
        AND product = 1''')
    products = cur.fetchall()
    return render_template(
        'show_userpage.html', title="Getränkeliste", user=user,
        products=products, balance=user_balance,
        user_list=user_list)
...
def action_buy(username, valuablename):
    ...
    cur.execute('SELECT rowid, price FROM valuable WHERE product=1 AND name=?',
                [valuablename])
    ...
    cur.execute(
        'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) VALUES  (?, ?, ?, ?, ?)',
        [app.config['STORAGE_ACCOUNT'][0], user['account_id'],
         valuable['rowid'], 1, transaction_id])
    if not user['direct_payment']:
        cur.execute(
            'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) VALUES  (?, ?, ?, ?, ?)',
            [user['account_id'], None, app.config['MONEY_VALUABLE_ID'],
             valuable['price'], transaction_id])
    else:
        cur.execute(
            'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) VALUES  (?, ?, ?, ?, ?)',
            [None, app.config['CASH_IN_ACCOUNT'][0],
             app.config['MONEY_VALUABLE_ID'], valuable['price'],
             transaction_id])
    db.commit()
...
def transfer_money(username):
    ...
    cur.execute(
        'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) VALUES  (?, ?, ?, ?, ?)',
        [user['account_id'], to_user['account_id'],
         app.config['MONEY_VALUABLE_ID'], amount, transaction_id])
    db.commit()
...
def edit_userprofile(username):
    ...
    if request.method == 'GET':
        ...
        cur = db.execute(
            ...
            account_from.name AS from_name, from_id,
            account_to.name AS to_name, to_id, amount, valuable.unit_name,
            valuable.name AS valuable_name, valuable_id
            FROM `transaction` JOIN transfer
            ON `transaction`.rowid = transfer.transaction_id
        ...
        return render_template('user_profile.html',
                               title="Benutzerprofil " + user['name'],
                               user=user, transactions=transactions,
                               balance=balance, return_to_userpage=True)
    else:  # request.method == 'POST':
        if not user['allow_edit_profile']:
            abort(403)

        if request.form['mail'] == '' or not EMAIL_REGEX.match(
                request.form['mail']):
        ...
        if request.files['image'] and request.files['image'].filename != '':
            ...
            filename = 'users/' + randomword(10) + '_' + secure_filename(
                image.filename)
            ...
            if float(im.size[0]) / float(im.size[1]) > 3.0 / 4.0:  # crop width
                new_width = int(im.size[0] * (( float(im.size[1]) * 3.0 ) / (
                    float(im.size[0]) * 4.0)))
            ...
            elif float(im.size[0]) / float(
                    im.size[1]) < 3.0 / 4.0:  # crop height
                new_height = int(im.size[1] * (
                    ( float(im.size[0]) * 4.0 ) / ( float(im.size[1]) * 3.0 ) ))
        ...
        if request.files['image'] and user['image_path']:
            ...
            os.unlink(
                os.path.join(app.config['UPLOAD_FOLDER'], user['image_path']))

        flash(u'Benutzerprofil erfolgreich aktualisiert!')
        return redirect(
            url_for('edit_userprofile', username=request.form['name']))


@app.route('/user/add', methods=['POST', 'GET'])
def add_user():
    if request.method == 'GET':
        return render_template('add_user.html', title="Benutzer hinzufügen",
                               admin_panel=True)
    else:  # request.method == 'POST'
        db = get_db()
        cur = db.cursor()
        ...
        if image and allowed_file(image.filename):
            ...
            # cut/crop image if not 3:4 ratio
            if float(im.size[0]) / float(im.size[1]) > 3.0 / 4.0:  # crop width
                new_width = int(im.size[0] * (
                    ( float(im.size[1]) * 3.0 ) / ( float(im.size[0]) * 4.0 ) ))
                left = int(im.size[0] / 2 - new_width / 2)
                im = im.crop((left, 0, left + new_width, im.size[1]))
                flash(u'Image had to be cropped to 3:4 ratio, sorry!')
            elif float(im.size[0]) / float(
        ...
        cur.execute(
            'INSERT INTO user (name, mail, account_id, image_path) VALUES (?, ?, ?, ?)',
            [request.form['name'], request.form['mail'], cur.lastrowid,
             filename])
        db.commit()
        return redirect(
            url_for('edit_userprofile', username=request.form['name']))
...
def add_to_account(username):
    ...
    cur = db.cursor()
    cur.execute(
        'SELECT account_id FROM user WHERE active=1 AND name=? AND direct_payment=0',
        [username])
    user = cur.fetchone()
    ...
    cur.execute(
        ...
        'VALUES  (?, ?, ?, ?, ?)',
        [None, user['account_id'], app.config['MONEY_VALUABLE_ID'], amount,
         transaction_id])
    cur.execute(
        'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) ' +
        'VALUES  (?, ?, ?, ?, ?)',
...
def sub_from_account(username):
    ...
    cur.execute(
        'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id)' +
        'VALUES  (?, ?, ?, ?, ?)',
        [user['account_id'], None, app.config['MONEY_VALUABLE_ID'], amount,
         transaction_id])
    cur.execute(
        'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) ' +
        'VALUES  (?, ?, ?, ?, ?)',
        [app.config['CASH_IN_ACCOUNT'][0], None,
         app.config['MONEY_VALUABLE_ID'], amount, transaction_id])
    db.commit()
...
def cancle_transaction(username, transaction_id):
    ...
    cur.execute('INSERT INTO `transaction` (datetime, comment) VALUES (?, ?)',
                [datetime.now(),
                 'Storno von ' + str(transaction_id) + ' durch ' + username])
    cancle_transaction_id = cur.lastrowid
    for t in transfers:
        cur.execute(
            'INSERT INTO transfer (from_id, to_id, valuable_id, amount, transaction_id) ' +
            'VALUES  (?, ?, ?, ?, ?)',
            [t['to_id'], t['from_id'], t['valuable_id'], t['amount'],
             cancle_transaction_id])
    db.commit()
